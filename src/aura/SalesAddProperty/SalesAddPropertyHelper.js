({
    getUnits : function(component, helper) {
        ...
        var showRecentlyAddedUnit = component.get('v.selectedRecentlyAdded');
       // if(component.get("v.selectedprojects") != null && component.get("v.selectedprojects") != ''){
       ...
       // }
        ...
        action.setParams({
            ...
            'project' : component.get("v.selectedprojects"),
            ...
            "recentlyAdded" : showRecentlyAddedUnit,
        });
                ...
                if(finalResult == null){
                    ...
                    resultData.forEach(function(record){
                       ...
                       /* if (record.Master_Community__c !== null && record.Master_Community__c !== undefined) {
                        ...
                        if(record.Unit_type__c == 'Villa'){
                            if (record.Cluster__c !== null && record.Cluster__c !== undefined) {
                                record.ClusterName = record.Cluster__r.Name;
                                record.clusterName = record.Cluster__r.Name;
                            }  
                        }
                        else if(record.Unit_type__c == 'Apartment' || record.Unit_type__c == 'Plot'){
                            if (record.Floor__c !== null && record.Floor__c !== undefined) {
                                record.ClusterName = record.Floor__r.Name;
                                record.clusterName = record.Floor__r.Name;
                            }
                        }
                        ...
                        if (record.Unit_theme__c !== null && record.Unit_theme__c !== undefined) {
                            record.UnitThemeName = '/'+record.Unit_theme__c;
                            record.unitThemeName = record.Unit_theme__r.Name;
                        }
                        ...
                        if(amount !== null && amount !== undefined){
                            var amountstr = formatter.format(amount);
                            //var amountstr = helper.formatCurr(amount);
                            //var amountstr = amount.toString();
                            record.Amountincurrency = amountstr;
                            record.convertedcurrency = amountstr;
                        }else{
                            record.Amountincurrency ='AED 00.0';
                            record.convertedcurrency ='AED 00.0';
                        } 
                    ...
                    component.set("v.totalpages",Math.ceil( finalResult.totalUnits / component.get("v.pageSize")));
    ...
    sortData: function (cmp, fieldName, sortDirection) {
     ...
     callProjects: function(component, event) {  
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var projectsMap = [];
                for(var key in result){
                    projectsMap.push({key: key, value: result[key]});
                }
                console.log('projectsMap is '+ projectsMap);
    ...
    callUnitType: function(component, event) {  
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var typesMap = [];
                for(var key in result){
                    typesMap.push({key: key, value: result[key]});
                }
                console.log('UnitTypeMap is '+ JSON.stringify(typesMap));
                component.set("v.UnitTypeMap", typesMap);
            }
        });
        $A.enqueueAction(action);
    },
